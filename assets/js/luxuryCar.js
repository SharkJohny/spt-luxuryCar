...
function createUpsaleButton(img, text, position, value, type, price2, prefix, texts) {
  ...
  if (prefix) {
    priceHTML = `<div class="price">od ${NumToPrice(priceText[0])} / ks</div><div class="save" data-save="${save}">${texts.you_will_save} a\u017E ${NumToPrice(save)}</div>`;
  }
...
function calculateStandartPrice(diference2) {
  ...
  const price2 = Number(
    $("span.calculated-price").text().replace(/[^0-9]/g, "")
  );
